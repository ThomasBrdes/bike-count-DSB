...
def _merge_holidays_week_end(X):
    ...
    X['is_holiday'] = X['date'].apply(
        is_holiday) | X['date'].apply(is_school_holiday)
...
def _merge_Curfews_lockdowns_COVID(X):
    ...
    curfew_periods = [
        {"range": pd.date_range(
            start="2020-10-17", end="2020-10-29"), "start": 21, "end": 6},
        {"range": pd.date_range(
            start="2021-01-16", end="2021-03-20"), "start": 18, "end": 6},
        {"range": pd.date_range(
            start="2021-03-20", end="2021-05-19"), "start": 19, "end": 6},
        {"range": pd.date_range(
            start="2021-05-19", end="2021-06-09"), "start": 21, "end": 6},
        {"range": pd.date_range(
            start="2021-06-09", end="2021-06-20"), "start": 23, "end": 6},
    ]
...
def _merge_indicators_COVID(X):
    ...
    df_ext = pd.read_csv(file_path, usecols=columns_COVID,
                         parse_dates=["date"])
    df_ext['date'] = pd.to_datetime(df_ext['date']).astype('datetime64[us]')
...
def _merge_road_accidents_by_year(year, start_date, end_date):
    ...
    columns_CARACT = ["Num_Acc", "jour", "mois",
                      "an", "hrmn", "dep", "com", "adr"]
    ...
    data_CARACT = pd.read_csv(
        data_CARACT_path, sep=";", usecols=columns_CARACT)
    data_USAGERS = pd.read_csv(
        data_USAGERS_path, sep=";", usecols=columns_USAGERS)
    ...
    data_CARACT['date'] = data_CARACT['an'].astype(str) + '-' + data_CARACT['mois'].astype(str).str.zfill(
        2) + '-' + data_CARACT['jour'].astype(str).str.zfill(2) + ' ' + data_CARACT['hrmn'].str[:2] + data_CARACT['hrmn'].str[2:]
    data_CARACT['date'] = pd.to_datetime(
        data_CARACT['date'], format='%Y-%m-%d %H:%M').astype('datetime64[us]')
    data_CARACT.drop(['jour', 'mois', 'an', 'hrmn'], axis=1, inplace=True)
    data_CARACT = data_CARACT[(data_CARACT['date'] >= start_date) & (
        data_CARACT['date'] <= end_date)]
    ...
    merged_CARACT_USAGERS['grav'] = merged_CARACT_USAGERS['grav'].map(
        severity_mapping)
    ...
    merged_CARACT_USAGERS = pd.merge(merged_CARACT_USAGERS.drop(
        'grav', axis=1).drop_duplicates('Num_Acc'), aggregated_data, on='Num_Acc')
    ...
    df_accidents = merged_CARACT_USAGERS[merged_CARACT_USAGERS['Num_Acc'].isin(
        data_VEHICULES['Num_Acc'])]
...
def _merge_road_accidents(X):
    ...
    # Reset index
    X["orig_index"] = np.arange(X.shape[0])
    X = pd.merge_asof(X.sort_values("date"), df_accidents[[
                      "date", "Max_Grav_accidents", "Count_accidents"]].sort_values("date"), on="date")
...
def get_estimator():
    ...
    numeric_cols = ['t', 'u', 'rr3', 'is_holiday', 'rea', 'rad', 'hosp', 'year',
